...
const Bookmark = () => {
    const { posts_list, bookmarkItems, addToBookmark, removeFromBookmark } = useContext(StoreContext);
    return (
        <div className='mt-10'>
            <div className='shadow-lg'>
                <div className='grid grid-cols-5 items-center text-lg p-5 bg-red-300 text-white font-semibold rounded-t-md'>
                ...
                {posts_list.map((item, index) => {
                    ...
                    if (bookmarkItems[item._id] > 0) {
                        return (
                            <div className=''>
                                <div key={index} className='grid grid-cols-5 items-center text-gray-500 text-lg py-2 px-5'>
                                    ...
                                    <button onClick={() => removeFromBookmark(item._id)} className='bg-red-200 rounded-md px-2 py-1 text-white max-w-fit hover:bg-red-300'>Remove</button>
                                </div>
                                <hr />
                            </div>
                        )
